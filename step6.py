...
import threading
import collections
import socket # NEW: For UDP peer discovery
...
# Get device name from environment variable, or use a default
# You should set this environment variable on each Raspberry Pi:
# e.g., export RPI_DEVICE_NAME="RaspberryPi_A"
DEVICE_NAME = os.getenv("RPI_DEVICE_NAME", "RaspberryPi_Unknown")
# Get this Pi's own IP address
# This is an approximation and might not work in complex network setups
# For more robustness, consider tools like `netifaces` or specific network configuration.
try:
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    s.connect(("8.8.8.8", 80)) # Connect to a public IP to get local IP used for outbound connection
    MY_IP_ADDRESS = s.getsockname()[0]
    s.close()
except Exception:
    MY_IP_ADDRESS = "127.0.0.1" # Fallback if cannot determine IP
print(f"[{get_jst_timestamp_ms()}] System: Device Name set to '{DEVICE_NAME}', IP: {MY_IP_ADDRESS}")

# --- Peer Discovery Configuration ---
PEER_DISCOVERY_PORT = 5001 # UDP port for peer discovery
PEER_BROADCAST_INTERVAL_SEC = 10 # How often to broadcast presence
PEER_TIMEOUT_SEC = 30 # How long to keep a peer in list if no response

# Dictionary to store discovered peers: {ip_address: {"name": device_name, "last_seen": timestamp}}
discovered_peers = {}
    ...
    try:
        with data_lock: # It MUST acquire the lock here, as callers might not hold it.
            global current_state, initial_time_raw, initial_time_display, final_time_raw, \
                    ...
                    discovered_peers # NEW: Reset discovered_peers too
            current_state = STATE_WAITING_FOR_START
            initial_time_raw = None
            initial_time_display = None
            final_time_raw = None
            ...
            discovered_peers.clear() # NEW: Clear discovered peers
    ...
    with data_lock: # Acquire lock to safely read and modify shared state variables
        if current_state == STATE_WAITING_FOR_START:
            if current_hinban_being_processed is not None:
                ...
                print(f"[{timestamp}] CALLBACK: START_SWITCH (GPIO{channel}) activated for '{current_hinban_being_processed}'. Initial time recorded.")
            else:
                print(f"[{timestamp}] CALLBACK: START_SWITCH (GPIO{channel}) activated, but no hinban set. Ignoring cycle start.")
        else:
...
def discovery():
    """
    Returns basic information about this Raspberry Pi for discovery by clients.
    """
    timestamp = get_jst_timestamp_ms()
    # It's generally safe to access DEVICE_NAME and MY_IP_ADDRESS without the data_lock
    # as they are set once at startup and not modified dynamically.
    print(f"[{timestamp}] API: Discovery request received from {request.remote_addr}. Responding with device name.")
    return jsonify({
        "status": "success",
        "device_name": DEVICE_NAME,
        "pi_ip_address": MY_IP_ADDRESS,
        "message": "This is a Raspberry Pi production monitoring backend."
    })

# NEW: Endpoint to get list of discovered peers
@app.route('/network-peers', methods=['GET'])
def get_network_peers():
    """
    Returns the list of other Raspberry Pis discovered on the local network.
    """
    timestamp = get_jst_timestamp_ms()
    with data_lock: # Protect access to discovered_peers
        # Convert dictionary to list of values for easier JSON serialization
        peers_list = list(discovered_peers.values())
    print(f"[{timestamp}] API: Network peers request received. Returning {len(peers_list)} peers.")
    return jsonify({
        "status": "success",
        "my_device": {"name": DEVICE_NAME, "ip": MY_IP_ADDRESS},
        "peers": peers_list
    })
            ...
            with data_lock:
                ...
                if current_state == STATE_WAITING_FOR_START:
                    ...
                    if mr_a_state == 1 and mr_b_state == 1 and mr_c_state == 1:
                        print(f"[{get_jst_timestamp_ms()}] GPIO Loop: All machine ready signals (GPIO{MACHINE_READY_A_PIN}, GPIO{MACHINE_READY_B_PIN}, GPIO{MACHINE_READY_C_PIN}) detected HIGH. State transition to PRODUCT_RELEASE.")
...
# NEW: Peer Discovery Thread
def run_peer_discovery_loop():
    """
    Function to run a UDP-based peer discovery loop.
    Each Pi broadcasts its presence and listens for others.
    """
    broadcast_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    broadcast_socket.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
    # Allows other applications to bind to the same port
    broadcast_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    broadcast_socket.settimeout(1.0) # Non-blocking listen with timeout

    # Bind to receive broadcast messages
    try:
        broadcast_socket.bind(('', PEER_DISCOVERY_PORT))
        print(f"[{get_jst_timestamp_ms()}] Peer Discovery: UDP socket bound to port {PEER_DISCOVERY_PORT}.")
    except Exception as e:
        print(f"[{get_jst_timestamp_ms()}] Peer Discovery: ERROR binding UDP socket: {e}")
        return # Cannot proceed with discovery
    ...
    try:
        while True:
            try:
                ...
                if current_time - last_broadcast_time > PEER_BROADCAST_INTERVAL_SEC:
                    message = f"RPI_DISCOVERY|{MY_IP_ADDRESS}|{DEVICE_NAME}".encode('utf-8')
                    try:
                        # Send to broadcast address (e.g., 192.168.1.255 for 192.168.1.x)
                        # For simplicity, sending to 255.255.255.255, which is global broadcast.
                        # A more robust approach would derive the subnet's broadcast address.
                        broadcast_socket.sendto(message, ('<broadcast>', PEER_DISCOVERY_PORT))
                        print(f"[{get_jst_timestamp_ms()}] Peer Discovery: Broadcasted presence.")
                    except Exception as e:
                        print(f"[{get_jst_timestamp_ms()}] Peer Discovery: ERROR sending broadcast: {e}")
                    last_broadcast_time = current_time

                # Listen for incoming messages
                try:
                    data, addr = broadcast_socket.recvfrom(1024) # Buffer size 1024 bytes
                    message = data.decode('utf-8')
                    peer_ip = addr[0]

                    if message.startswith("RPI_DISCOVERY|") and peer_ip != MY_IP_ADDRESS:
                        parts = message.split('|')
                        if len(parts) == 3:
                            discovered_ip = parts[1]
                            discovered_name = parts[2]

                            with data_lock: # Protect discovered_peers
                                if discovered_ip not in discovered_peers:
                                    print(f"[{get_jst_timestamp_ms()}] Peer Discovery: Discovered new peer: {discovered_name} at {discovered_ip}")
                                elif discovered_peers[discovered_ip]["name"] != discovered_name:
                                    print(f"[{get_jst_timestamp_ms()}] Peer Discovery: Peer {discovered_ip} name changed to {discovered_name}.")
                                discovered_peers[discovered_ip] = {
                                    "name": discovered_name,
                                    "ip": discovered_ip, # Store IP within the dict entry for convenience
                    ...
                    print(f"[{get_jst_timestamp_ms()}] Peer Discovery: ERROR receiving message: {e}")

                # Clean up stale peers
                with data_lock: # Protect discovered_peers
                    stale_peers = [ip for ip, info in discovered_peers.items() if current_time - info["last_seen"] > PEER_TIMEOUT_SEC]
                    for ip in stale_peers:
                        print(f"[{get_jst_timestamp_ms()}] Peer Discovery: Peer {discovered_peers[ip]['name']} ({ip}) timed out.")
                        del discovered_peers[ip]

                time.sleep(0.5) # Small delay to prevent excessive CPU usage

            except KeyboardInterrupt:
                print(f"\n[{get_jst_timestamp_ms()}] Peer Discovery: Exiting due to Ctrl+C...")
                break
            except Exception as e:
                print(f"[{get_jst_timestamp_ms()}] Peer Discovery: !!! AN UNEXPECTED ERROR OCCURRED IN PEER DISCOVERY THREAD: {type(e).__name__}: {e} !!!")
                traceback.print_exc()
    finally:
        print(f"[{get_jst_timestamp_ms()}] Peer Discovery: Closing UDP socket.")
        broadcast_socket.close()
        print(f"[{get_jst_timestamp_ms()}] Peer Discovery: UDP socket closed.")
...
if __name__ == '__main__':
    ...
    # NEW: Peer discovery thread
    peer_discovery_thread = threading.Thread(target=run_peer_discovery_loop, name="PeerDiscoveryThread")

    gpio_thread.start()
    flask_thread.start()
    peer_discovery_thread.start() # NEW: Start peer discovery thread
    ...
    try:
        ...
        while True:
            ...
            if not flask_thread.is_alive():
                ...
                break # Exit main loop
            # NEW: Monitor peer discovery thread
            if not peer_discovery_thread.is_alive():
                print(f"[{get_jst_timestamp_ms()}] System: Peer Discovery thread has terminated unexpectedly! Peer discovery is down.")
                # Don't break here, as the main functionality might still work without peer discovery
    except KeyboardInterrupt:
        ...
        # NEW: Join peer discovery thread
        if peer_discovery_thread.is_alive():
            print(f"[{get_jst_timestamp_ms()}] System: Waiting for Peer Discovery thread to finish...")
            peer_discovery_thread.join(timeout=2)
